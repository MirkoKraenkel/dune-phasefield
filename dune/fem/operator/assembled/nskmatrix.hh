#ifndef NSKJACOBIANOPERATOR_HH
#define NSKiJACOBIANOPERATOR_HH
      ...
      for( size_t jj=0 ;  jj < numScalarBf ; ++jj)
        {
          ...
          for( size_t kk = 0 ; kk < dimDomain ; ++kk )
            {
              ...
              for( size_t ll = 0; ll<dimDomain; ++ll)
                ...
                //(v,rho,mu*)
               flux[ 1+kk ][ 0 ]+=0.5*phi[ jj ][ 0 ]*duImEx[ 1 + dimDomain ][ kk ];
               //(v,rho*,mu)
               flux[ 1+kk ][ dimDomain + 1 ]+=imexFactor_*vuMid[ 0 ]*dphi[ jj ][ 0 ][ kk ];
          ...
          //(mu, v_kk)
          for( size_t kk = 0 ; kk < dimDomain ; ++kk )
            flux[ dimDomain+1 ][ 1+kk]-=2*vu[ 1+kk ]*phi[ jj ][ 0 ]*0.25;
      
          //(mu,mu)
          flux[ dimDomain+1 ][ dimDomain+1 ]=imexFactor_*phi[ jj ][ 0 ];
          ...
          for( size_t kk = 0 ; kk < dimDomain ; ++kk )
            {
              //(mu, gradsigma)
              flux[ dimDomain+1 ][ dimDomain+2+kk ]+=model_.delta()*0.5*dphi[ jj ][ 0 ][ kk ];
              //(sigma,rho))
              flux[ dimDomain+2+kk ][ 0 ]-=dphi[ jj ][ 0 ][ kk ];
              //(sigma,sigma)
              flux[ dimDomain+2+kk ][ dimDomain+2+kk]=phi[ jj ][ 0 ];
            }  
          DiffusionType du;
