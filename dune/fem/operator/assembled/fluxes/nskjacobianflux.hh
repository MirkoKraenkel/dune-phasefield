#ifndef PHASEFIELD_NSK_JACOBIAN_FLUX_HH
#define PHASEFIELD_NSK_JACOBIAN_FLUX_HH

#include <dune/common/fvector.hh>

#include "../nskfilter.hh"
template<class Model>
class NskJacobianFlux
{
  //typedefs
...
public:
  NskJacobianFlux(const ModelType& model):
    model_(model),
    ...
    for(int ii = 0; ii < dimDomain ; ++ii )
      {
        ...
        gLeft[ dimDomain + 1 ][ dimDomain + 2 +ii ]=-model_.delta()*laplaceFlux;
      } 
    ...
    for(int ii = 0; ii < dimDomain ; ++ii )
      {
        ...
        gLeft[ dimDomain + 1 ][ 2*dimDomain + 2 +ii ]=-model_.delta()*laplaceFlux;
#else
        // F_{3.3}(\sigma^+)\cdot n
        gLeft[ dimDomain + 1 ][ dimDomain + 2 +ii ]=-model_.delta()*laplaceFlux;
#endif
...
::numericalFlux( const DomainType& normal,
                ...
                FluxRangeType& gRight) const
  {
      ...
      for(int ii = 0; ii < dimDomain ; ++ii )
        {
          ...
          //(\phi*,\tau_j,v_i)
          gLeft[ 1 + ii ][ dimDomain + 1 ]=jump[ dimDomain + 1 ]*normal[ ii ]*factorImp_;
          //(\phi_j,\tau*,v_i)
          gLeft[ 1 + ii ][ dimDomain + 1] =factorImp_*normal[ ii ]*vuEnImEx[ dimDomain + 1 ];
          gRight[ 1 + ii ][ dimDomain + 1 ]=-factorImp_*normal[ ii ]*vuEnImEx[ dimDomain + 1 ];
          ...
          //(mu,sigma))
          gLeft[ dimDomain + 1 ][ dimDomain + 2+ii ]=-model_.delta()*laplaceFluxLeft;
          gRight[ dimDomain + 1 ][dimDomain + 2+ii ]=-model_.delta()*laplaceFluxRight;

          gLeft[ dimDomain + 1 ][ dimDomain + 1]=-model_.delta()*factorImp_*penaltyTerm;
          gRight[ dimDomain + 1 ][ dimDomain + 1]=model_.delta()*factorImp_*penaltyTerm;

          //F_4
          //(rho^+-\rho^-)\normal*0.5
          gLeft[ dimDomain + 2 + ii ][ 0 ]=normal[ ii ]*0.5;
          gRight[ dimDomain + 2 + ii ][0 ]=normal[ ii ]*-0.5;
       }
      ...
      gRight[ 0 ][ 0 ]=0.5*factorImp_*vNormalNb;
       //[[mu]]
      gLeft[ 0 ][ dimDomain+1 ]=factorImp_*numViscMu_*area;
      gRight[ 0 ][ dimDomain+1 ]=-factorImp_*numViscMu_*area;
      //[[rho]]
...
template< class Model >
template< class ScalarType >
void NskJacobianFlux<Model>::diffPhiDiffusionFlux ( const DomainType& normal,
                                                const double penaltyFactor,
                                                const RangeType& vuEn,
                                                const RangeType& vuNb,
                                                const JacobianRangeType& duEn,
...
template< class Model >
template< class ScalarType >
void NskJacobianFlux<Model>::diffPhiBoundaryFlux ( const DomainType& normal,
                                                const double penaltyFactor,
                                                const RangeType& vuEn,
                                                const JacobianRangeType& duEn,
                                                const ScalarType& phiEn,
