    ...
    JacobianRangeType& adu) const
{
  ...
  model_.muSource(Filter::rho(vuOld),Filter::rho(vuOld),Filter::phi(vuOld),dFdrho);
...
#if RHOMODEL
    //\sigma^n*\sigma^{n-1}
    sigmasqr+=Filter::sigma( vuOld , ii )*Filter::sigma( vuOld , ii );
#endif
  }
...
#if RHOMODEL && LAMBDASCHEME
    Filter::alpha( avu, ii )=Filter::alpha(vu,ii)-model_.h2(Filter::rho(vuMid))*Filter::sigma(vuMid, ii);
#endif
 }
