...
for cp in acelemcouplings1d:
  ...
  newline='elementCouplings_.push_back( std::make_pair('+str(v1)+','+str(v2)+'));//('+cp[0]+','+cp[1]+')\n'
  f.write(newline)
  i=i+1
f.close()
...
for cp in acinterseccouplings1d:
  v1=acvariables1d[ cp[0] ]
  v2=acvariables1d[ cp[1] ]
  newline='intersectionCouplings_.push_back( std::make_pair('+str(v1)+','+str(v2)+'));//('+cp[0]+','+cp[1]+')\n'
  f.write(newline)
  i=i+1
f.close()
###################################################################################################################################
