 ...
 :public Dune::Fem::DifferentiableOperator < Jacobian >
{
  ...
  public: 
  LocalFDOperator( const MyOperatorType& myOperator):
  ...
  void setDeltaT( const double deltat) { myOperator_.setDeltaT_(deltat); }
  ...
  const DiscreteFunctionType& space() const { return myOperator_.space();}
  const ModelType& model() const { return model_: } 

  void jacobian(const DiscreteFunctionType &u, JacobianOperatorType &jOp) const;

  private:
  const MyOperatorType& myOperator_;
