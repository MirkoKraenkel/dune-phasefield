...
nu := proc (phi) options operator, arrow; phi end proc; W := proc (phi) options operator, arrow; phi*log(phi)+(1-phi)*log(1-phi)-(1/2)*phi^2 end proc; dW := D(W); F := proc (rho, phi) options operator, arrow; theta*rho*W(phi)+nu(phi)*F1(rho)+(1-nu(phi))*F0(rho) end proc; Pressure := proc (rho, phi) options operator, arrow; nu(phi)*p1(rho)+(1-nu(phi))*p0(rho) end proc; Potential := proc (rho, phi) options operator, arrow; 1.*W(phi)/delta+nu(phi)*G1(rho)+(1-nu(phi))*G0(rho) end proc;
solrho := proc (x) options operator, arrow; exp((Const-nu(x)*(b-f)-f)/(nu(x)*(a-e)+e)) end proc; Const := G1(sol1); solproc := makeproc(solrho(x), x); evalRho := optimize(solproc); C(evalRho, filename = outstring, ansi);
Fproc := makeproc(F(rho, phi), [rho, phi]); helmholtz := optimize(Fproc); C(helmholtz, filename = outstring, ansi);
S := proc (rho, phi) options operator, arrow; diff(F(rho, phi), phi) end proc; Sproc := makeproc(S(rho, phi)/delta, [rho, phi]); reactionSource := optimize(Sproc); C(reactionSource, filename = outstring, ansi);
dphiS := proc (rho, phi) options operator, arrow; diff(S(rho, phi), phi) end proc; dphiSproc := makeproc(dphiS(rho, phi)/delta, [rho, phi]); dphireactionSource := optimize(dphiSproc); C(dphireactionSource, filename = outstring, ansi);
