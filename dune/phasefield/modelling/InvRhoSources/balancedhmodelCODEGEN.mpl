...
tht1 := proc (x) options operator, arrow; reactionSource(rhosol(x), solphi(x)) end proc; thetasol1 := makeproc(tht1(x), x); thetasol1 := optimize(thetasol1); C(thetasol1, filename = outstring2, ansi);
tht2 := proc (x) options operator, arrow; -delta*laplacephi(x)/rhosol(x)+delta*gradrho(x)*gradphi(x)/(rhosol(x)*rhosol(x)) end proc; thetasol2 := makeproc(tht2(x), x); thetasol2 := optimize(thetasol2); C(thetasol2, filename = outstring2, ansi);
...
RsV := proc (x) options operator, arrow; -gradphi(x)*(thetasol1(x)+thetasol2(x))+rhosol(x)*gradmu(x) end proc;
