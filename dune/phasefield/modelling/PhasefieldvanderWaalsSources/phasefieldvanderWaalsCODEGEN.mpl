restart; with(codegen); with(plots); outstring2 := "phasefieldvanderWaalsRho.cc"; outstring := "phasefieldvanderWaalsmaple.cc"; with(CurveFitting);
FvdW := proc (rho) options operator, arrow; rho*(-rho+(8/27)*theta*log(rho/(1-rho))) end proc; theta := .85; alpha := 0;
gvdW := proc (rho) options operator, arrow; diff(FvdW(rho), rho) end proc; 
pp := proc (rho) options operator, arrow; -FvdW(rho)+rho*gvdW(rho) end proc; 
pvdW := makeproc(pp(rho), rho); GvdW := makeproc(gvdW(rho), rho);
sols := fsolve([pvdW(x) = pvdW(y), gvdW(x) = gvdW(y)], {x = 0 .. 3, y = .4 .. 1}); 
r1 := solve(sols[1]); r2 := solve(sols[2]); 
dpvdW := proc (rho) options operator, arrow; diff(pvdW(rho), rho) end proc; 
mwg := proc (rho) options operator, arrow; GvdW(r1)*rho-pvdW(r2) end proc; 
F2 := proc (rho) options operator, arrow; FvdW(rho)-mwg(rho) end proc; 
G2 := D(F2);
psols := solve(dpvdW(x), x); pp1 := psols[1]; pp2 := psols[2]; Gv2 := D(F2); 
p2 := proc (rho) options operator, arrow; -F2(rho)+rho*Gv2(rho) end proc; 
C2 := makeproc(diff(pvdW(rho), rho), [rho]);
F1 := proc (rho) options operator, arrow; i*rho/(rho-1)+c*rho*log(rho)+(b-c)*rho+d end proc;  
g1 := D(F1); g1(r2); g1(pp2); 
p1 := proc (rho) options operator, arrow; -F1(rho)+rho*g1(rho) end proc; 
G1 := makeproc(g1(x), x); G1(r2); 
C1 := makeproc(diff(p1(rho), rho), [rho]);
sol3 := fsolve({G2(x) = 0}, x = .3 .. .4); 
r3 := solve(sol3[1]);
...
F0 := proc (rho) options operator, arrow; h*rho^2+e*rho*log(rho)+(f-e)*rho+g end proc; 
g0 := D(F0); p0 := proc (rho) options operator, arrow; -F0(rho)+rho*g0(rho) end proc; 
G0 := makeproc(g0(x), x); 
C0 := makeproc(diff(p0(rho), rho), [rho]);
...
W := proc (phi) options operator, arrow; 2*phi^4+2*(2*alpha-2)*phi^3+2*(-3*alpha+1)*phi^2+2*alpha end proc; 
...
F := proc (rho, phi) options operator, arrow; A*W(phi)/delta+nn(phi)*F1(rho)+(1-nn(phi))*F0(rho) end proc; 
rhosol := proc (phi) options operator, arrow; phi*r2+(1-phi)*r1 end proc;
...
SSproc := makeproc(Sproc(rho, phi, old, .5*(phi+old)), [rho, phi, old]); 
reactionSource := optimize(SSproc); 
C(reactionSource, filename = outstring, ansi);
Fbulk := proc (rho, phi) options operator, arrow; beta*(nn(phi)*F1(rho)+(1-nn(phi))*F0(rho)) end proc;
dphiS := proc (rho, phi, old) options operator, arrow; diff(SSproc(rho, phi, old), phi) end proc; 
dphiSproc := makeproc(dphiS(rho, phi, old, mid), [rho, phi, old, mid]); 
...
dphichemicalPotential := optimize(dphimuproc); 
C(dphichemicalPotential, filename = outstring, ansi);
# 
Pproc := makeproc(P2(rho, phi), [rho, phi]); 
pressure := optimize(Pproc); 
C(pressure, filename = outstring, ansi);
wavespeed := proc (rho, phi) options operator, arrow; sqrt(diff(Pressure(rho, phi), rho)) end proc; 
wproc := makeproc(simplify(wavespeed(rho, phi)), [rho, phi]); 
a := optimize(wproc); 
C(a, filename = outstring, ansi);
...
for k to numP do CC[k] := (k-1)/(numP-1); B[k] := fsolve(Potential(x, (k-1)/numP) = G1(bubblesol), x = 0.1e-1 .. 1) end do; pair := proc (x, y) options operator, arrow; [x, y] end proc; rhoI := makeproc(Spline(CC, B, x), [x]);
evalRho1 := optimize(rhoI); evalRho := prep2trans(evalRho1); C(evalRho, filename = outstring2, ansi);
