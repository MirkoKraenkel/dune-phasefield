...
solrho1 := proc (x) options operator, arrow; exp((Const-nn(x)*(f-b)-b)/(nn(x)*(e-c)+c)) end proc; Const := G1(sol1); solproc1 := makeproc(solrho1(x), x); evalRho1 := optimize(solproc1); C(evalRho1, filename = outstring2);
...
sols := fsolve([p1(rho)+inttanh/(.25) = p0(x), G1(rho) = G0(x)], {rho = 0 .. 15, x = 0 .. 15}); bubblesol := solve(sols[1]); bubblesol2 := solve(sols[2]); ml := proc (a) options operator, arrow; bubblesol end proc; mwpliq := makeproc(ml(x), x);
...
solrho := proc (x) options operator, arrow; exp((Const-nn(x)*(f-b)-b)/(nn(x)*(e-c)+c)) end proc; solproc := makeproc(solrho(x), x); evalRho := optimize(solproc); C(evalRho, filename = outstring2, ansi);
