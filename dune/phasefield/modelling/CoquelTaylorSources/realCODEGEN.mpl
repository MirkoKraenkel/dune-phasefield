...
A := 0.1e-2; alpha := 0; inttanh := int(sqrt(2*A*W(x)), x = 0. .. 1.);
sols := fsolve([p1(rho)-inttanh/(.25) = p0(x), G1(rho) = G0(x)], {rho = 0 .. 1, x = 0 .. 1});
sols := fsolve([p1(rho)+(1/25)*inttanh = p0(x), G1(rho) = G0(x)], {rho = 0 .. 1, x = 0 .. 1}); bubblesol := solve(sols[1]); bubblesol2 := solve(sols[2]); ml := proc (a) options operator, arrow; bubblesol end proc; mwpliq := makeproc(ml(x), x);
