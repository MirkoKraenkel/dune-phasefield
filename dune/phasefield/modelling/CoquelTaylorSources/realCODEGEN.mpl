...
G1 := D(F11); G0 := D(F00); 
p1 := proc (rho) options operator, arrow; -F11(rho)+rho*G1(rho) end proc; 
p0 := proc (rho) options operator, arrow; -F00(rho)+rho*G0(rho) end proc;
sols := fsolve([p1(rho) = p0(x), G1(rho) = G0(x)], {rho = 0 .. 4, x = 0 .. 4}); 
sol1 := solve(sols[1]); sol2 := solve(sols[2]); 
mwg := proc (x) options operator, arrow; G1(sol1)*x-p1(sol1) end proc; 
F1 := F11-mwg; F0 := F00-mwg;
...
solrho := proc (x) options operator, arrow; exp((Const-nn(x)*(b-f)-f)/(nn(x)*(c-e)+e)) end proc; 
Const := G1(sol1); solproc := makeproc(solrho(x), x); evalRho := optimize(solproc);
...
sols := fsolve([p1(rho)+(1/25)*inttanh = p0(x), G1(rho) = G0(x)], {rho = 0 .. 1, x = 0 .. 1}); 
