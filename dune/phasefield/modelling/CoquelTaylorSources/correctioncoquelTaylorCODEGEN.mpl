...
solrho := proc (x) options operator, arrow; exp((Const-nn(x)*(b-f)-f)/(nn(x)*(c-e)+e)) end proc; Const := G1(sol1); solproc := makeproc(solrho(x), x); evalRho := optimize(solproc); C(evalRho, filename = outstring2);
Fbulk := proc (rho, phi) options operator, arrow; nn(phi)*F1(rho)+(1-nn(phi))*F0(rho) end proc;
Fproc := makeproc(F(rho, phi)-beta*Fbulk(solproc(phi), phi), [rho, phi]); helmholtz := optimize(Fproc); C(helmholtz, filename = outstring, ansi);
# 
