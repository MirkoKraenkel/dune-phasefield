...
inline void PhasefieldPhysics< 2,Thermodynamics >
                        ...
                        double& reaction ) const
  {
    assert( cons[0] > 1e-20 );
		...
		double phi=cons[phaseId];
		phi/=rho;
    double dxphi=du[phaseId][0];
    double dyphi=du[phaseId][1];
...
inline void  PhasefieldPhysics< 2,Thermodynamics >
  ::analyticalFlux ( const RangeType& u, JacobianRangeType& f ) const
  {
    ...
    f[phaseId][0] = phi*u[1]; // \phi*\rho*vx
    f[phaseId][1] = phi*u[2]; // \phi*\rho*vy
  }
...
::stiffSource( const DomainType& x,
    ...
    double phi=u[phaseId]*rho_inv;
    double dphix=jacU[phaseId][0]-phi*jacU[0][0];
    double dphiy=jacU[phaseId][1]-phi*jacU[0][1];
    dphix*=rho_inv;
    dphiy*=rho_inv;
...
inline void PhasefieldPhysics< 2, Thermodynamics >
                ...
                ThetaJacobianRangeType& diff ) const
  {
    ...
    diff[1][1]=-delta()*thermoDynamics_.h2( u[0] )*du[phaseId][1];
  }
...
inline double PhasefieldPhysics< 2, Thermodynamics >
  ...
  RangeFieldType phi=u[phaseId]/u[0];
  RangeFieldType c = thermoDynamics_.a( u[0] , phi )*n.two_norm2();
