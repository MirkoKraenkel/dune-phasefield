	      ...
	      JacobianRangeType& diff ) const 
{
  ...
  const double rhodxphi = rho_inv*(du30 - phi*du00); //dx(rho*phi)-phi*dx(rho)=rho*dx(phi);
  const double rhodzphi = rho_inv*(du31 - phi*du01);
  ...
  // 4th row
#if LOWMACH`
  diff[3][0] =delta*rhodxphi;           diff[3][1] =delta* rhodzphi;
#else
  diff[3][0] =rhodxphi;                 diff[3][1] = rhodzphi;
#endif
}
