...
7
  ...
  /**\brief PressureLaw for the first Phase
   */
  template< class RangeType>
  inline void phase1Law(const RangeType& cons,double& p,double& dphi);
  {
    double rho=cons[0];
    double phi=cons[dimDomain+1];
    phi/=rho;
 ...
 /**\brief PressureLaw for the second Phase
   */
  template< class RangeType>
  inline void phase2Law(const RangeType& cons,double& p,double& dphi);
  {
    double rho=cons[0];
    double phi=cons[dimDomain+1];
    phi/=rho;
