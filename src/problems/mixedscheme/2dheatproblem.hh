...
:: evaluate( const double t, const DomainType& arg, RangeType& res ) const 
{
  ...
  dFdphi*=thermodyn_.deltaInv();
   ...
   double phi=-0.05*cosx*cosy+0.5;
   res[dimension+1]=phi;
  //res[dimension+1]=tanx;
  if( dimRange > dimDomain+2)
  {
    //mu
    res[dimension+2]=0.5*v*v+thermodyn_.chemicalPotential(rho,phi);
    //tau
    res[dimension+3]=-thermodyn_.delta()*8*M_PI*M_PI*cosx*cosy*0.05+thermodyn_.reactionSource(rho,phi);//+dFdphi;
  ...
  if(dimension==1)
    ...
    { 
      res[dimension+4]=2*M_PI*sinx*cosy*0.05;
      res[dimension+5]=2*M_PI*siny*cosx*0.05;
    }
}
else
{
